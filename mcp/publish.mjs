 ...
 * how a reputation transaction is built (no duplication of `src/`).
 *
 * Signing is pluggable via the library's Signer abstraction. Two modes are
 * supported, chosen by environment:
 *
 ...
 *                                  an external wallet (Nautilus / ErgoPay) to
 *                                  sign. No private key in the agent. (default)
 *     CELAUT_ADDRESS    (required)  the P2PK address whose UTXOs fund the tx.
 *
 * Common:
 *   CELAUT_EXPLORER_API           explorer base (default mainnet).
 */
...
// Type NFT IDs — must match `server.mjs` / `src/lib/api.ts`.
...
function describeResult(result) {
  ...
  // Unsigned mode: hand the EIP-12 tx back for an external wallet to sign.
  return {
    submitted: false,
